# Taken from: https://github.com/sbi-dev/sbi/blob/main/sbi/diagnostics/tarp.py
"""
...
def l2(x: torch.Tensor, y: torch.Tensor, axis=-1) -> torch.Tensor:
    """
    ...
    Args:
        ...
        axis (int, optional): The axis along which to calculate the L2 distance.
                Defaults to -1.
    Returns:
        torch.Tensor: A tensor containing the L2 distance between x and y along the
                specified axis.
    """
...
def run_tarp(
    ...
    Args:
        ...
        posterior_samples: posterior samples obtained from a probabilistic regression method.
...
def _run_tarp(
    ...
    ecp = torch.cat([torch.Tensor([0]), ecp])

    return ecp, alpha_grid


def get_tarp_references(thetas: torch.Tensor) -> torch.Tensor:
...
def plot_tarp(
    ecp: torch.Tensor, alpha: torch.Tensor, title: Optional[str] = None
...
def run_tarp_evaluation(
    ...
    Args:
        ...
        output_dir: Directory where to save the TARP results.
    Returns:
        A dictionary containing TARP results.
    """
    ...
    ecp, alpha = run_tarp(
        thetas,
        posterior_samples,
        references=references,  # will be calculated automatically.
    )

    # Similar to SBC, we can check then check whether the distribution of ecp is close to
    # that of alpha.
    atc, ks_pval = check_tarp(ecp, alpha)
    print(f"ATC: {atc:.4f}, should be close to 0.0")
    print(f"KS p-value: {ks_pval}, should be larger than 0.05")

    # Or, we can perform a visual check.
    fig, ax = plot_tarp(ecp, alpha)
    fig.savefig(output_dir / "ecp.png", dpi=400)
    plt.close(fig)

    # Save TARP statistics
    tarp_results = {
        "ecp": ecp,
        "alpha": alpha,
        "atc": atc,
