...
def plot_multiple_relative_performances(
    ...
    fig, axs = plt.subplots(1, len(tasks), figsize=(28, 8))
    ...
    fig.savefig(
        os.path.join(output_dir, "relative_performances_ode_solvers.pdf"),
        format="pdf", bbox_inches="tight", dpi=400
    )
    fig.savefig(
        os.path.join(output_dir, "relative_performances_ode_solvers.png"),
...
if __name__ == "__main__":
    script_start = time()
    print("\nRUN ABLATION STUDY ON THE IMPACT OF ODE SOLVERS\n")
