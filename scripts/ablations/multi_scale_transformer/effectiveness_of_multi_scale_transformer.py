...
def plot_relative_performance(
    ...
    fig, axs = plt.subplots(1, 5, figsize=(32, 8), sharey=True)
...
if __name__ == "__main__":
    script_start = time()
    print("\nRUN ABLATION STUDY ON THE EFFECTIVENESS OF MULTI-SCALE TRANSFORMER\n")
    ...
    if args.tasks is None or args.tasks == "regression":
        ...
        regression_relative_perf = measure_relative_performance(
            dfs=regression_all_samples_dfs,
            filter_columns=filter_columns['regression']
        )
        performance_gain['regression'] = regression_relative_perf.iloc[0].to_dict()
        print("Done!\n\n")
    ...
    if args.tasks is None or args.tasks == "calibration":
        ...
        performance_gain['calibration'] = calibration_relative_perf.iloc[0].to_dict()
    ...
    if args.tasks is None or args.tasks == "coverage":
        ...
        performance_gain['coverage'] = coverage_relative_perf.iloc[0].to_dict()
    ...
    if args.tasks is None or args.tasks == "distribution_metrics":
        ...
        mmd_expanded_dfs = []
        for jsd_df, mmd_df in zip(jsd_aggregate_dfs, mmd_aggregate_dfs):
            ...
            if jsd_df is not None and mmd_df is not None:
                ...
                mmd_expanded_dfs.append(mmd_expanded)
        
        # Merge the two dataframes on 'parameters' column
        distribution_metrics_dfs = []
        for jsd_df, mmd_df in zip(jsd_aggregate_dfs, mmd_expanded_dfs):
            if jsd_df is not None and mmd_df is not None:
                merged_df = pd.merge(
                    jsd_df, 
                    mmd_df, 
                ...
                distribution_metrics_dfs.append(merged_df)
        ...
        distribution_metrics_relative_perf = measure_relative_performance(
            dfs=distribution_metrics_dfs,
            filter_columns=filter_columns['distribution_metrics']
        )
        performance_gain['distribution_metrics'] = distribution_metrics_relative_perf.iloc[0].to_dict()
    ...
    if args.tasks is None or args.tasks == "log_probs":
        ...
        performance_gain['log_probs'] = log_probs_relative_perf.iloc[0].to_dict()
