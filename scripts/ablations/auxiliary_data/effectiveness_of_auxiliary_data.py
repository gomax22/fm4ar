...
def make_estimator_palette(estimator_names):
    ...
    colors = [mcolors.to_hex(cmap(1)) for i in range(len(estimator_names))]
...
def plot_multiple_relative_performances(
    ...
    # fig, axs = plt.subplots(1, len(tasks), figsize=(32, 8))
    ...
    fig.savefig(
        os.path.join(output_dir, "relative_performances_aux_data.pdf"),
        format="pdf", bbox_inches="tight", dpi=400
    )
    fig.savefig(
        os.path.join(output_dir, "relative_performances_aux_data.png"),
...
if __name__ == "__main__":
    script_start = time()
    print("\nRUN ABLATION STUDY ON THE EFFECTIVENESS OF AUXILIARY DATA\n")
